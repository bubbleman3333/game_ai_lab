// ブロブチェインのオンライン対戦の部屋。準備完了 → カウントダウン → 対戦 → 結果 → もう一度。
// 部屋・通信の仕組みはテトリスと共通（api/online/）。
...
import { isTouchDevice, useViewportWidth } from '../../../lib/useViewport'
import { MiniField } from '../components/BlobCanvas'
import { BlobPlayer } from '../components/BlobPlayer'
import { BlobTouch } from '../components/BlobTouch'
import { BlobOnlineMatch } from '../game/onlineMatch'
...
export function BlobOnlineRoomPage() {
  ...
  const [match, setMatch] = useState<BlobOnlineMatch | null>(null)
  ...
  // 接続（部屋コードが変わったらつなぎ直す）
  useEffect(() => {
    const s = new MatchSocket(code, name, 'blob')
    s.onClose = (c) => {
    ...
    const off = s.subscribe((msg) => {
      switch (msg.type) {
        ...
        case 'start':
          setMatch((old) => {
            old?.finish()
            return new BlobOnlineMatch(s, msg.seed)
          })
          setOpponent(null)
          setResult(null)
  ...
  useEffect(() => () => match?.finish(), [match])

  const input = useMemo(() => (match ? new BlobInput(match.controller) : null), [match])
  useEffect(() => input?.attach(), [input])
  useEffect(() => (match ? attachBlobSounds(match.controller) : undefined), [match])
  ...
  useEffect(() => {
    if (phase === 'countdown' && left > 0) blobSounds.countdown(false)
    if (phase === 'playing') blobSounds.countdown(true)
  }, [left, phase])
  useEffect(() => {
    if (phase === 'result' && result?.win) blobSounds.win()
  }, [phase, result])

  const overlay =
    phase === 'countdown' ? <span className="countdown">{left || 'GO'}</span>
      : phase === 'result' && result ? <div className={`blob-result ${result.win ? 'win' : 'lose'}`}><div className="big">{result.win ? 'WIN' : 'LOSE'}</div></div>
        : undefined

  return (
    <div className="page blob-page">
      <h1>
        ブロブチェイン オンライン対戦 <span className="mono">{code}</span>
      </h1>
      <p className="muted">
        この URL を相手に送ると同じ部屋に入れます。<Link to="/blob/online">ロビーに戻る</Link>
      </p>
      ...
      {result && phase === 'result' && (
        <p className={result.win ? 'win' : 'lose'}>
          {result.win ? '勝ち！' : '負け…'}（{result.reason === 'disconnect' ? '相手の切断' : result.win ? '相手の盤が埋まった' : '盤が埋まった'}）
        </p>
      )}

      <div className="blob-arena versus">
        {match ? (
          <BlobPlayer controller={match.controller} title={me?.name ?? 'あなた'} drawRef={drawRef} overlay={overlay} />
        ) : (
          <div className="placeholder">両者が「準備完了」を押すと始まります</div>
        )}
        <div className="opponent blob-opponent">
          <div className="player-title">{other?.name ?? '相手'}</div>
          {opponent ? (
            <>
              <MiniField rows={opponent.rows} width={vw < 700 ? 110 : 170} dim={phase === 'result' && !!result?.win} />
              <dl className="stats">
                <dt>最大連鎖</dt><dd>{opponent.stats.lines}</dd>
                <dt>送った</dt><dd>{opponent.stats.attack}</dd>
      ...
      {input && isTouchDevice() ? <BlobTouch input={input} /> : <p className="key-help">{KEY_HELP}</p>}
    </div>
  )
}
