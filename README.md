...
| ポーカー | `/poker` AI と対戦・`/poker/stats` 強さ | 1 対 1 のノーリミット・ホールデム。**相手の手札が見えない**ので価値を学ぶやり方は使えず、後悔をニューラルネットに覚えさせる Deep CFR で学習 |
...
.\.venv\Scripts\python -m rl.poker.train_deep --run-name n1 # ポーカー（GPU + CPU 並列。Deep CFR）→ backend/rl/poker/README.md
