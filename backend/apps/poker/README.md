...
AI の ID は `heuristic`（学習なしのルールベース。いつでも使える）か `<学習名>:best` / `<学習名>:latest`。
学習した戦略が**まだ知らない場面**ではルールベースに任せる（`rl.poker.players.strategy_player`
の `fallback`）。学習の途中でも、急にでたらめを打つことがない。
