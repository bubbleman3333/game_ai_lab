"""使えるエアホッケー AI の一覧と、方策（JSON）の読み込み。

AI の ID:
    "heuristic"      学習なしの AI（ブラウザに組み込み済み）
    "<run>:best"     runs/airhockey/<run>/checkpoints/best.json
    "<run>:latest"   runs/airhockey/<run>/checkpoints/latest.json
"""
...
def _runs_dir() -> Path:
    return Path(settings.TRAINING_RUNS_DIR) / "airhockey"
...
def list_agents() -> list[AgentInfo]:
    ...
    # 学習した AI が学習なしの AI に勝ち越すまでは、学習なしを既定（先頭）にする
...
def _beats_heuristic(info: AgentInfo) -> bool:
    """その学習の最新の評価で、学習なしの AI への得点率が 5 割を超えているか。"""
    if info.run is None:
        return False
    ...
    try:
        ...
        return bool(best) and best[-1]["results"]["heuristic"]["win_rate"] > 0.5
