...
from .encoding import chain_potential, encode_many, shape_features
...
def reward_of(c: Candidate, cfg: RewardConfig) -> float:
    """1 手ぶんの報酬。中身の考え方は config.RewardConfig を参照。"""
    if c.dead:
        return cfg.death
    r = c.result
    value = cfg.alive + cfg.attack * r.sent + cfg.cancel * r.cancelled + cfg.all_clear * r.all_clear
    if r.chain:
        value += cfg.chain * r.chain * r.chain
        if r.chain < cfg.min_chain:
            value += cfg.small_chain  # 組みかけを壊す早撃ちへの罰
    return value
...
class NeuralAgent:
    ...
    lookahead=n にすると NEXT の組を n 個先まで読む（ビームサーチ。_beam_search）。
    ブロブチェインで見えている NEXT は 2 つなので lookahead は 2 まで意味がある。
    ...
    def load(cls, path: Path, device="cpu", lookahead: int = 0, beam: int = 8) -> "NeuralAgent":
        ...
        agent = cls(model, cfg.get("gamma", 0.98), reward, device, name=Path(path).stem,
...
class HeuristicAgent:
    """重み固定の評価関数。強さ比較の基準（学習なしでも常に使える AI）に使う。

    考え方は人が連鎖を組むときと同じで、
      「あと 1 個置けば何連鎖するか」を伸ばしながら、低く・平らに積む。
    危なくなったら（高く積み上がった・おじゃまが来ている）そこで連鎖を撃つ。
    """

    WEIGHTS = {
        "potential_chain": 9.0, "potential_score": 0.0015, "links": 0.7, "groups3": 0.8,
        "max_height": -2.2, "bumpiness": -1.4, "buried": -1.5, "sum_height": -0.25,
    }
    # 「撃つ」判断: 送るおじゃま 1 個の価値。危ないときほど大きくする
    FIRE_BASE = 0.8
    FIRE_DANGER = 2.5
    DANGER_HEIGHT = 9  # この高さを超えたら危ない
    ...
    def _score(self, c: Candidate) -> float:
        ...
        w = self.WEIGHTS
        danger = f["max_height"] >= self.DANGER_HEIGHT or c.pending_after >= 12
        value = (
            w["potential_chain"] * chain + w["potential_score"] * score
            + w["links"] * f["links"] + w["groups3"] * f["groups3"]
            + w["max_height"] * f["max_height"] + w["bumpiness"] * f["bumpiness"]
            + w["buried"] * f["buried"] + w["sum_height"] * f["sum_height"]
        )
        fire = self.FIRE_DANGER if danger else self.FIRE_BASE
        return value + fire * (c.result.sent + c.result.cancelled) + 12.0 * c.result.all_clear
