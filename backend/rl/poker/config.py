...
# 浅い方を細かく刻んでいるのは、**退場が起きるのは浅いところだけ**だから。
# 情報集合の鍵の先頭が深さなので表が完全に分かれており、深さごとに別プロセスで学習できる
# （`train.py --depth k` を同時に走らせて `merge.py` でまとめる）。
STACK_DEPTHS_BB = (10, 20, 35, 50, 75, 100)
