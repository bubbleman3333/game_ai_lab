...
class RewardConfig:
    ...
    attack: float = 1.0
    death: float = -5.0
    # 相手をトップアウトさせたときの報酬（自己対戦のときだけ効く）。**既定は 0（使わない）**。
    #
    # 狙いは「今トドメを刺す」を「後で同じだけ殴る」より高く評価させることだったが、
    # 今の実装には欠陥がある。相手は自分の手番で死ぬので、勝った側の「最後の局面」は
    # 勝ちと因果関係がない（たまたまそこにいただけ）。そこへ終端の +win を教えると、
    # 同じ盤面が「ふつうの局面」にも「終端で +5 の局面」にもなり、価値関数が壊れる。
    # 実際 v5-opponent は自己対戦に入った途端、火力 70 → 24〜46 まで落ちて best を更新できなくなった。
    # 直すなら、トドメの火力を出した手そのものの報酬に足す形にすること。
    win: float = 0.0
