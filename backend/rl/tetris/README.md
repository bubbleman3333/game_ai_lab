...
- 対戦画面の AI は `config/settings.py` の `TETRIS_AI_LOOKAHEAD`（環境変数でも変えられる）を使う。
...
| `lookahead=4`（**対戦画面の既定**） | 約 200ms | 178.2 | 100% | 217.5 |

1 手あたりの火力は、なし 0.34 → `lookahead=4` で 0.59（おじゃまありでは 0.45 → 0.73）。
1 手 0.2 秒かかるので、AI の速さを 4 PPS 以上にすると、考える時間で速さが頭打ちになる。
